    ...
    def add_knowledge(self, cell, count):
        ...
        for i in range(max(cell[0] - 1, 0), min(cell[0] + 2, self.height)):
            for j in range(max(cell[1] - 1, 0),  min(cell[1] + 2, self.width)):
                if (i, j) != cell:
                    if (i, j) not in (self.mines and self.safes):
                        neighbourCells.add((i, j))
        
        self.knowledge.append(Sentence(neighbourCells, count))

        #4).
        for sentence in self.knowledge:
            
            cells = copy.copy(sentence.cells)
        ...
        for i in range(len(self.knowledge)):
            ...
            for j in range(len(self.knowledge)):
                ...
                if (self.knowledge[i].cells).issubset(self.knowledge[j].cells):
                    newSentenceCells = self.knowledge[j].cells - self.knowledge[i].cells
                    newSentenceCount = self.knowledge[j].count - self.knowledge[i].count
